...
const contractAddress = require(`../.openzeppelin/unknown-${networkId}.json`)
  .proxies[0].address;
const permissionsContractAddress = require(
  `../../permissions/.openzeppelin/unknown-${networkId}.json`,
).proxies[0].address;
...
module.exports = async (deployer) => {
  ...
  const hasScope = await permissionsInstance.checkAddressScope(
    instance.address,
    'coupon:burn',
  );
  console.log(
    `Metadata Registry Contract at ${instance.address} add "coupon:burn" result: ${hasScope}`,
  );
};
