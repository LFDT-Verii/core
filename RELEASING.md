...
- `production` validates the checked-in release notes for each selected group, fails if any target group tag or GitHub Release already exists, publishes the exact package versions from the selected commit with the npm `latest` dist-tag, creates group git tags, and creates GitHub Releases from those notes. Selecting `credentialinghub` also publishes `lfdecentralizedtrust/verii-credentialing-hub:<package version>` and moves `lfdecentralizedtrust/verii-credentialing-hub:latest` to that image.
