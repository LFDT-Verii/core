...
- Prerelease publishing from `main` uses `.github/release-train.json` to decide each group's current release train.
...
For prerelease trains, record the intended stable target for every release group in `.github/release-train.json`:
...
The prerelease workflow validates that `targetVersion` matches the configured `bump` from the checked-in package manifest, then publishes a disposable prerelease for that stable target, for example `2.0.1-pre.<epoch>.0`. Update the release train before publishing prereleases for a different patch, minor, or major line.
...
1. Generates a temporary Nx version plan from `.github/release-train.json` for the selected release groups.
...
- `prerelease` publishes disposable prerelease versions from the selected commit according to `.github/release-train.json`, with a `pre.<epoch-seconds>` prerelease id and the npm `prerelease` dist-tag.
